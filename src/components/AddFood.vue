  ...
  <div class="add-item-container">
    <form @submit.prevent="">
      ...
      <div class="image-previews">
        <div v-for="(photo, index) in newFood.photos" :key="photo.url" class="image-preview-container">
      ...
      <div class="form-group">
        ...
        <input type="text" id="item-name" v-model="newFood.name" autocomplete="off" required>
      </div>

      <div class="form-group">
        <label for="item-name" class="form-label">Name of Restaurant</label>
        <input type="text" id="item-name" v-model="newFood.shop" autocomplete="off" required>
      </div>

      <!-- Brief Description Field -->
      <div class="form-group">
        <label for="brief-description" class="form-label">Brief Description</label>
        <textarea id="brief-description" class="description-input" v-model="newFood.description" required rows="2"></textarea>
      </div>
      ...
      <div class="form-group">
        ...
        <input type="text" id="price" v-model="newFood.price" 
        :disabled="isDonation" 
      ...
      <div class="form-group">
        ...
        <textarea id="location-address" class="location-input" v-model="newFood.location" required rows="1"></textarea>
      </div>
  
      <div class="form-group">
        <label for="item-type" class="form-label">Type of Listing</label>
        <select id="item-type" v-model="newFood.listingType" required>
      ...
      <div class="message-input">
        ...
        <v-btn class="ma-2" style="width: 240px; height: 50px; position:absolute; right:40px" @click="validateFood()">Confirm & Add</v-btn>
      </div>
...
computed: {
  isDonation() {
    return this.newFood.listingType === 'donate';
  },
},
watch: {
  'newFood.listingType'(newType) {
    if (newType === 'donate') {
      this.newFood.price = ''; // Clear the price
    }
  },
},
...
methods: {
  ...
  async calculateDistance(foodLocation) {
    ...
    async addFood() {
      ...
      this.newFood.uid = user.uid;
      const distance = await this.calculateDistance(this.newFood.location);
      if (distance) {
        this.newFood.distance = distance;
      ...
      await addDoc(collection(db, 'foods'), this.newFood);
      this.resetForm();
  ...
  handlePhotoUpload(event) {
    ...
    reader.onload = async(e) => {
      ...
      try {
        ...
        this.newFood.photos.push({ url: downloadURL });
      } 
      catch (error) {
  ...
  removePhoto(index) {
    ...
    URL.revokeObjectURL(this.newFood.photos[index].url);
    // Remove the photo from the array
    this.newFood.photos.splice(index, 1);
  },
  validateFood() {
    ...
    if (
      ...
      (this.newFood.price === '' && this.newFood.listingType==='sell')||
      this.newFood.location === '' ||
      this.newFood.listingType === '' ||
      this.newFood.collectionMethod === ''
    ) {
  ...
  resetForm() {
    ...
    this.newFood.photos.forEach(photo => URL.revokeObjectURL(photo.url));
    this.newFood = {
      photos: [],
      name: '',
      shop: '',
